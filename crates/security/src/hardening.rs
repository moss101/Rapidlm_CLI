    ...
    for case in cases {
        ...
        match reader.read_frame(&acp::stdio::CancellationToken::new()) {
            ...
            Err(
                ...
                | StdioError::UnrecoverableFrame { .. }
                | StdioError::EmbeddedNewline
...
mod tests {
    ...
    fn acp_decoder_survives_hostile_inputs_without_panicking() {
        ...
        let bomb_line = format!(
            "{}{}",
            "{\"a\":".repeat(500),
            "1".to_owned() + &"}".repeat(500)
        );
    ...
    fn oversized_single_line_is_rejected_with_typed_bound_error() {
        ...
        big.push(b'\n');
        let mut reader = acp::stdio::FrameReader::new(&big[..], 1024).expect("bounds");
        let outcome = reader.read_frame(&acp::stdio::CancellationToken::new());
        assert!(matches!(
            outcome,
            Err(acp::stdio::StdioError::FrameTooLarge { .. })
        ));
    }
}
