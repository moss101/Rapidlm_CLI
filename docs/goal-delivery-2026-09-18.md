...
| The stale-workspace gate actually fires | **not yet** | `Supervisor::current_identity` is set once at `start` and never refreshed (`set_current_identity` has no production caller), and attestations copy it verbatim, so the gate still compares a value with itself — only the value is now meaningful. `require_workspace_identity` remains `false`. Turning the gate on needs the identity recomputed *after* the work, which is not wired |
