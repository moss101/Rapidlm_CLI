...
| Ordering: checks gate visibility, the command runs after | stated | The transaction's *required* checks (parent revision still current, every preimage still holding) gate visibility — a stronger guarantee than the per-file check they replace. The caller's check **command** is deliberately not a verification hook: a hook runs before the checkout is written, so a command that reads the real tree would test the unmodified files. It runs after materializing, as it always has |
...
| A run actually recovers as paused | done | `OrchestrationSnapshot` is now serializable (every field type already was; the container simply had not opted in, with `paused_from` defaulted so an older snapshot still loads). `RunState.orchestration` carries it, and `Supervisor::recover` reduces it back — pausing a run that was mid-flight and leaving a terminal one alone, so a finished run is never reopened by a restart (`a_recovered_run_reduces_from_a_serialized_snapshot_and_comes_back_paused`, `a_terminal_run_is_recovered_as_it_was_with_nothing_to_pause`, `a_resumed_verified_run_recovers_its_supervisor_as_paused`) |
