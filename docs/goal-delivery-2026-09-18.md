...
| A run actually recovers as paused | **not yet** | `pause`/`resume_paused` have no production caller: nothing reconstructs a `Supervisor` after a restart, because that needs the snapshot reducer below. The machinery is in place and tested; the caller is not |
...
| The supervisor snapshot's own reducer | pending | `replay` rebuilds the graph; rebuilding an `OrchestrationSnapshot` from the stream needs the full record set writing to the ledger, which these record *types* enable but do not yet wire. `OrchestrationSnapshot` also carries no serde implementation, so it does not yet cross a process boundary at all |
