...
| Publication through `TransactionManager` | done | `TransactionManager` could not write a checkout at all — its own `begin_transaction` says "Parent checkout is not written", and nothing materialized an overlay — so the missing capability was added rather than worked around: `TransactionManager::materialize` writes a committed publication's overlay into a real tree (`materialize_writes_a_committed_publication_into_the_checkout`, and it refuses a receipt it never issued). `agent_views::integrate` now stages the child's changes as a `SemanticPatch`, previews the merge, `begin_transaction` → required checks → `commit_transaction`, and the `CommitReceipt` (parent revision + patch hash + change count) is the publication receipt the journal records the materializing write against (`publication_goes_through_a_workspace_transaction_and_its_receipt`, which also proves a non-UTF-8 file survives) |
| Ordering: the command runs after materializing | stated | The caller's check **command** is deliberately not a verification hook: a hook runs before the checkout is written, so a command that reads the real tree would test the unmodified files. It runs after materializing, as it always has |
| What the transaction's required checks actually verify | corrected | An earlier version of this row claimed they "re-verify the parent revision and every preimage". They do not: `VerificationCheck::ParentRevision` compares the preview's recorded revision with the view it was copied from — a value against itself — and `Preimage` compares view ids, re-hashing nothing. Preimages are validated exactly once, when the patch is staged. The per-file `git diff --quiet <base>` check in `integrate` was **not** replaced and is still the check that compares against the child's base commit |
...
| Criterion | Status | Evidence |
|---|---|---|
| Candidate, test and environment digests | computed; only the workspace one is wired | `apps/rapid/src/digests.rs`: the workspace (HEAD plus every deviation, content included, NUL-separated so a crafted filename cannot forge a status line), the check definition (command plus the content of files it names), the environment (toolchain versions). Each fails closed — an identity that cannot be established is `unavailable:` and compares equal to nothing, not even another failure. `check_definition_digest` and `environment_digest` have no callers yet; they exist for the check receipts GVS-003's arms need |
...
| The stale-workspace gate actually fires | done for `goal claim` | The gate was off because turning it on would have compared a constant with itself. Now `require_workspace_identity: true`, and `run_claim` refreshes `current_identity` from the tree *after* verification, so acceptance compares the workspace the verifier attested about with the one being accepted (`a_claim_arms_the_stale_workspace_gate_against_a_real_tree_identity`, revert-cycled; the refusal itself is `stale_workspace_cannot_accept`). `rapid run --orchestration verified` still leaves it off — its identity is the `watch`-glob digest, not the tree |
...
| Criterion | Status | Evidence |
|---|---|---|
| The state machine can express a paused run | done | `OrchestrationState::Paused` with a `Pause` transition; `Supervisor::pause` records the interrupted phase and `resume_paused` returns to exactly it, refusing to guess when it was not recorded and refusing a terminal or `Paused` target. Terminal states cannot pause (`a_run_pauses_from_any_live_phase_and_never_from_a_terminal_one`). There is deliberately no `Resume` transition — the target phase is data, not a fixed edge |
| The snapshot reduces back into a paused run | done | `OrchestrationSnapshot` is serializable (every field type already was; the container had not opted in), `RunState.orchestration` persists it, and `Supervisor::recover` reduces it back — pausing a mid-flight run and leaving a terminal one alone, so a finished run is never reopened by a restart (`a_recovered_run_reduces_from_a_serialized_snapshot_and_comes_back_paused`, `a_terminal_run_is_recovered_as_it_was_with_nothing_to_pause`, `a_resumed_verified_run_recovers_its_supervisor_as_paused`). A snapshot this binary cannot decode is read as absent rather than failing the whole run state (`an_unreadable_snapshot_does_not_make_the_whole_run_state_unloadable`) |
| A run *continues* from its recovered supervisor | **not yet** | `VerifiedRun::recovered_state` returns the recovered state and nothing more — `rapid run` still opens a fresh supervisor and mirrors the run-state file. Recovering the state is the reducer; continuing a run from it needs the run loop to drive a restored `Supervisor`, which is not wired |
...
| Versioned records with additive evolution | done (types only; no writer yet) | `agent-runtime` `orchestration::records`: a self-describing `record: "<kind>/v<n>"` envelope, unknown fields ignored (old reader reads a new record; new reader defaults what an old one lacks), foreign records and future majors as **typed skips**, bodies bounded at 64 KiB. `protocol::CandidateId` added |
| The supervisor snapshot's own reducer | done for the snapshot; the event-stream reduction is still pending | `Supervisor::recover` is the reducer: a serialized snapshot becomes a live, paused run. It reduces from the **snapshot** the run persisted, not yet from the event stream — rebuilding one purely from events still needs the full record set writing to the ledger, which GVS-004's record *types* enable but do not yet wire |

### Known limits in this batch, stated rather than discovered later

- **At-most-once is enforced across processes only by git.** `find_by_fingerprint` and `prepare` are separate transactions and `operation_journal` has no unique constraint on `(session_id, fingerprint)`, so two concurrent publications of the same effect can both prepare and both call the apply closure; the second `git apply` then fails because the patch is already applied. The journal contributes idempotency *within* a process and across restarts, not mutual exclusion — a `UNIQUE(session_id, fingerprint)` claim-by-insert is the fix and is not in this batch.
- **`recover` is scoped to the caller's own effect.** The publication session is project-wide, so a recovery pass sees other agents' in-flight records; it leaves every record whose fingerprint is not the caller's untouched, because this caller holds no evidence about them. That means a crashed publication is settled only when the same effect is attempted again — an operation nobody retries stays `Uncertain`, visibly, rather than being guessed at by an unrelated patch.
