...
| `continue_on_length` | 0–8 | no (default 0) | When an answer ends at its output limit, up to this many follow-on requests hand the answer so far back and ask for the rest; the parts are one message. Each request is a `model.continued` ledger record and counts against the turn's token budget; `rapid exec` says so on stderr. A continuation is sent only while the answer can still grow within what one message holds (64 KiB, a part taken as `max_tokens` × 4 bytes); a part that would overflow it, or a reply proposing tool calls, leaves the answer as it was. `0`: the answer ends where the limit cut it |
