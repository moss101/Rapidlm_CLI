...
not guessed, before wiring `shell_exec` through it. | Landlock + Seatbelt + child seccomp, named profiles | Docker/Podman only (also no native Linux sandbox — this is a real RapidLM opportunity: `GvisorBackend`'s syscall mediation is a stronger property than Qwen's container-only story) | `crates/sandbox/src/backend.rs` (`SandboxExecResult`, done) + `apps/rapid` (wiring, not done) | P0 (output capture done; wiring remains) | L |
