...
|---|---|---|---|---|---|---|
| 1 | `apps/rapid` never wires `SandboxManager` — Linux/Windows shell_exec gets no sandboxing at all today (not "weaker than Landlock," *none*), while a working `ContainerBackend`/`GvisorBackend` sit unused | Landlock + Seatbelt + child seccomp, named profiles | Docker/Podman only (also no native Linux sandbox — this is a real RapidLM opportunity: `GvisorBackend`'s syscall mediation is a stronger property than Qwen's container-only story) | Add `sandbox` as an `apps/rapid` dependency; register `HostRestrictedBackend` + `ContainerBackend` + `GvisorBackend` (when `health()` passes) behind `SandboxManager`; route `shell_exec`'s non-macOS `sandbox: true` path through `prepare`/`exec`/`destroy` instead of leaving it a no-op | P0 | M |
...
| 5 | Subagent results return as flat text (`SubagentRunner::run(...) -> Result<String, String>`, `apps/rapid/src/exec_tools.rs:493`), discarding structure `AgentExecutionRequest`'s own outcome already has (status, tool-call count) | Not established | Structured completion via `functionResponse`/tool-result parts, but no dedicated child-result schema either — this is a genuine RapidLM opportunity (Phase 2 §2.2) | `apps/rapid/src/exec_tools.rs` (`SubagentRunner` trait) + `interactive.rs` (`LiveSubagentRunner`) | P1 | M |
...
| # | Gap | Grok Build | Qwen Code | Where it lands | Sev | Effort |
|---|---|---|---|---|---|---|
| 7 | Hooks cover only `pre_tool_use`/`post_tool_use`, blocking | 15 events, blocking semantics | ~15 events across 4 executor types (`command`/`http`/`function`/LLM-judged `prompt`), parallel by default | `apps/rapid/src/hooks.rs` | P1 | M |
